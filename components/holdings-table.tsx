...
import { deleteFund, updateAllotment } from "@/app/actions";
...
export function HoldingsTable({ holdings }: { holdings: Holding[] }) {
  return (
    <div className="table-shell">
      <table>
        <thead><tr><th>Fund</th><th>Invested</th><th>Current value</th><th>Returns</th><th>NAV</th><th><span className="sr-only">Actions</span></th></tr></thead>
        <tbody>{holdings.map((fund) => (
          <tr key={fund.id}>
            <td><div className="fund-name"><span>{fund.short_name.slice(0, 2).toUpperCase()}</span><div><strong>{fund.short_name}</strong><small>{fund.category} · {friendlyDate(fund.purchase_date)}</small></div></div></td>
