        ...
        <table>
          ...
          <tbody>{visible.map((fund) => (
            <tr key={fund.id}>
              ...
              <td data-label="Invested">{inr.format(fund.invested_amount)}</td>
              <td data-label="Current value"><strong>{inr.format(fund.currentValue)}</strong>{fund.isEstimated ? <small className="estimate">Estimated</small> : null}</td>
              <td data-label="Returns" className={fund.gain >= 0 ? "positive" : "negative"}><strong>{fund.gain >= 0 ? "+" : ""}{inr.format(fund.gain)}</strong><small>{fund.gainPercent >= 0 ? "+" : ""}{fund.gainPercent.toFixed(2)}%</small></td>
              <td data-label="NAV / Price"><strong>{fund.currentNav ? inr.format(fund.currentNav) : "—"}</strong><small>{fund.currentNavDate ?? "Unavailable"}</small></td>
              <td><details className="row-menu"><summary aria-label="More actions"><MoreHorizontal /></summary><div className="row-popover">{fund.instrumentType === "mutual_fund" ? <><p>Add exact allotment</p><form action={updateAllotment} className="allotment-form"><input type="hidden" name="id" value={fund.id}/><label>Units<input name="units" type="number" min="0" step="0.000001" defaultValue={fund.units ?? undefined} placeholder={fund.effectiveUnits?.toFixed(4)} required/></label><label>Purchase NAV<input name="purchase_nav" type="number" min="0" step="0.0001" defaultValue={fund.purchase_nav ?? undefined} placeholder={fund.effectivePurchaseNav?.toFixed(4)} required/></label><SubmitButton className="save-action" pendingLabel="Saving..."><Save size={14}/> Save exact values</SubmitButton></form></> : <><p>Edit holding</p><form action={updateEtfHolding} className="allotment-form"><input type="hidden" name="id" value={fund.id}/><label className="wide">NSE symbol<input name="symbol" defaultValue={fund.symbol ?? undefined} placeholder="e.g. GOLDIETF" required/></label><label>Quantity<input name="quantity" type="number" min="0" step="0.000001" defaultValue={fund.effectiveUnits ?? undefined} required/></label><label>Average price<input name="avg_price" type="number" min="0" step="0.0001" defaultValue={fund.effectivePurchaseNav ?? undefined} required/></label><SubmitButton className="save-action" pendingLabel="Saving..."><Save size={14}/> Save exact values</SubmitButton></form></>}<form action={deleteHolding.bind(null, fund.id, fund.instrumentType)}><SubmitButton className="delete-action" pendingLabel="Removing..."><Trash2 size={14}/> Remove holding</SubmitButton></form></div></details></td>
            </tr>
          ))}</tbody>
        </table>
