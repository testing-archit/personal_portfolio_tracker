  ...
  return (
    <>
      ...
      <dialog ref={dialog} className="fund-dialog">
        ...
        <form action={addFund} className="fund-form" onSubmit={() => dialog.current?.close()}>
          ...
          <div className="dialog-actions wide"><button type="button" className="secondary-button" onClick={() => dialog.current?.close()}>Cancel</button><button type="submit" className="primary-button">Save investment</button></div>
        </form>
      </dialog>
    </>
  );
