...
import { enrichFund } from "@/lib/nav";
...
export default async function Dashboard() {
  ...
  const { data } = await supabase.from("funds").select("*").order("invested_amount", { ascending: false });
  const holdings = await Promise.all(((data ?? []) as Fund[]).map(enrichFund));
  ...
  return (
    <main className="dashboard-shell">
      <header className="topbar">
        ...
        <nav><a className="active" href="#overview">Overview</a><a href="#holdings">Holdings</a></nav>
      ...
      <section className="dashboard-content" id="overview">
        <div className="welcome-row"><div><p className="eyebrow">GOOD MORNING, {name.toUpperCase()}</p><h1>Your money, at a glance.</h1></div><div className="nav-status"><RefreshCw size={14}/><span>Latest NAV</span><strong>{latestDate ?? "Waiting for data"}</strong></div></div>
        {holdings.length === 0 ? (
          ...
          <>
            <section className="metric-grid">
              ...
              <article className="metric"><p>Total invested</p><strong>{inr.format(invested)}</strong><small>Across {holdings.length} direct-growth funds</small></article>
            ...
            <section className="insight-grid">
              <article className="panel chart-panel"><div className="panel-head"><div><p className="eyebrow">PORTFOLIO MOVEMENT</p><h2>{compactInr.format(current)}</h2></div><span className="period-chip">Since investment</span></div><PortfolioChart invested={invested} current={current}/></article>
            ...
            <section id="holdings" className="holdings-section"><div className="section-head"><div><p className="eyebrow">YOUR PORTFOLIO</p><h2>Holdings</h2></div><div className="estimate-key"><Clock3 size={14}/> Estimated values use purchase-date NAV until exact units are added.</div></div><HoldingsTable holdings={holdings}/></section>
          </>
        )}
      </section>
      <footer><span>Folio is for tracking only, not investment advice.</span><span>NAV data refreshes from public AMFI feeds.</span></footer>
    </main>
  );
}
